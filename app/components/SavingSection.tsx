...
export default function SavingSection() {
  ...
  const scenarios: StoreScenario[] = useMemo(
    () => [
      { name: "Магазин A", items: 1710, delivery: 149, promo: 120, total: 1739, badge: "лучше по цене" },
      { name: "Магазин B", items: 1650, delivery: 249, promo: 80, total: 1819, badge: "быстрее" },
      { name: "Магазин C", items: 1760, delivery: 99, promo: 0, total: 1859, badge: "доставка дешевле" },
    ],
    []
  );
      ...
      <div className="container-page">
        <div className="max-w-3xl">
          ...
          <p className="section-subtitle">
            Одна и та же корзина может стоить по-разному из-за цен, доставки и промо.
            Корзина считает итоговую стоимость и показывает, где реально выгоднее.
          </p>
        </div>
            ...
            <div className="relative">
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                ...
                <div className="text-xs text-slate-600">
                  *демо-цифры, позже заменим на реальные
                </div>
              </div>
              ...
              <div className="mt-6 space-y-4">
                {scenarios.map((s) => {
                  const pct =
                    maxTotal === minTotal
                      ? 100
                      : 100 - ((s.total - minTotal) / (maxTotal - minTotal)) * 35; // 65–100% ширина, красиво
                  const isBest = s.total === minTotal;

                  return (
                    <div key={s.name} className="rounded-2xl bg-white/50 border border-black/5 p-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {s.badge ? (
                            <span
                              ...
                              style={{
                                background: isBest ? "rgba(254,105,0,0.12)" : "rgba(0,0,0,0.04)",
                      ...
                      <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-slate-600">
                        <div className="rounded-xl bg-white/40 border border-black/5 px-3 py-2">
                          <div className="text-[11px] uppercase tracking-wide">Товары</div>
                          <div className="mt-1 font-semibold text-slate-900">{formatRUB(s.items)}</div>
                        </div>
                        <div className="rounded-xl bg-white/40 border border-black/5 px-3 py-2">
                          <div className="text-[11px] uppercase tracking-wide">Доставка</div>
                          <div className="mt-1 font-semibold text-slate-900">{formatRUB(s.delivery)}</div>
                        </div>
                        <div className="rounded-xl bg-white/40 border border-black/5 px-3 py-2">
                          <div className="text-[11px] uppercase tracking-wide">Промо</div>
                          <div className="mt-1 font-semibold" style={{ color: s.promo ? ORANGE : "rgba(15,23,42,0.55)" }}>
            ...
            <div className="relative">
              ...
              <div className="mt-6 rounded-2xl bg-white/50 border border-black/5 p-4">
                ...
                <div className="mt-1 text-sm text-slate-700">
                  Корзина показывает <span className="font-semibold">реальную итоговую стоимость</span>,
            ...
            <div className="inline-flex rounded-full border border-black/10 bg-white/50 p-1">
              <button
                ...
                className={[
                  ...
                  mode === "manual" ? "bg-white shadow-soft text-slate-900" : "text-slate-600 hover:text-slate-900",
                ].join(" ")}
              >
                Вручную
              </button>
              <button
                ...
                className={[
                  "rounded-full px-4 py-2 text-sm font-semibold transition",
                  mode === "korzina" ? "bg-white shadow-soft text-slate-900" : "text-slate-600 hover:text-slate-900",
                ].join(" ")}
              >
                С Корзиной
              </button>
            ...
            <div className="rounded-2xl bg-white/50 border border-black/5 p-5">
              ...
              <div className="mt-2 text-sm text-slate-700 leading-relaxed">
                {mode === "manual" ? (
                  ...
                  <>
                    Ты получаешь лучший итоговый чек без лишних действий: выбираешь
                    “дешевле / быстрее / удобнее” — и идёшь оформлять заказ.
                  </>
                )}
              </div>
        ...
        {items.map((t) => (
          <li key={t} className="flex gap-3">
            <span
              ...
              style={{ backgroundColor: accent ? ORANGE : "rgba(15,23,42,0.35)" }}
            />
            <span>{t}</span>
          </li>
        ))}
